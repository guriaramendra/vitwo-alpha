    ...
    if (!isset($_SESSION["logedCompanyAdminInfo"]["adminId"]) || !isset($_SESSION["logedCompanyAdminInfo"]["adminRole"])) {
        ...
        if ($adminRole != 1) {
            ...
            if (!in_array($currentPage, $commonAccessPages)) {
                $sqlAccesses = "SELECT `fldRoleAccesses` FROM `tbl_company_admin_roles` WHERE `fldRoleStatus`='active' AND `fldRoleKey`=" . $adminRole;
                $accesses = "";
...
function getAdministratorMenuSubMenu()
{
    ...
    $adminRole = $_SESSION["logedCompanyAdminInfo"]["adminRole"];
    if($adminRole==1){
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_company_admin_menu` WHERE `fldParentMenuKey`!=0 AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
    ...
    $parentMenuKeysArr=[];
    $sqlParentMenuKeys = "SELECT `fldParentMenuKey` FROM `tbl_company_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' GROUP BY `fldParentMenuKey`;";
    ...
    $sqlMenuList="SELECT * FROM `tbl_company_admin_menu` WHERE `fldMenuKey` IN (".$parentMenuKeys.") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
    if($qryMenuList=mysqli_query($dbCon,$sqlMenuList)){
        if (mysqli_num_rows($qryMenuList)>0) {
            ...
            while($rowMenuList=mysqli_fetch_assoc($qryMenuList)){
                ...
                $sqlSubMenuList="SELECT * FROM `tbl_company_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=".$parentMenuKey." ORDER BY `fldMenuOrderBy` ASC";
...
function addNewAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $fldRoleCompanyId = $_SESSION["logedCompanyAdminInfo"]["fldAdminCompanyId"];

        $sql = "SELECT * FROM `tbl_company_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleCompanyId`='" . $fldRoleCompanyId . "' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_company_admin_roles` SET
...
function updateAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_company_admin_roles` WHERE `fldRoleKey`=" . $roleKey . " AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 1) {

                $checkName = "SELECT * FROM `tbl_company_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleKey`!=" . $roleKey . " AND  `fldRoleStatus`!='deleted'";
                ...
                $ins = "UPDATE `tbl_company_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
...
function getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_company_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleKey`=" . $key . "";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorRoles()
{
    ...
    $returnData = [];
    $fldRoleCompanyId = $_SESSION["logedCompanyAdminInfo"]["fldAdminCompanyId"];
    $sql = "SELECT * FROM `tbl_company_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleCompanyId`=".$fldRoleCompanyId."";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorUsers()
{
    ...
    $returnData = [];
    $sql = "SELECT * FROM `tbl_company_admin_details`,`tbl_company_admin_roles` WHERE `tbl_company_admin_details`.`fldAdminRole`=`tbl_company_admin_roles`.`fldRoleKey` AND `tbl_company_admin_details`.`fldAdminCompanyId`='".$fldAdminCompanyId."' AND `tbl_company_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_company_admin_roles`.`fldRoleStatus`!='deleted'";
...
function getAdministratorUserDetails($key = null)
{
    ...
    $sql = "SELECT `tbl_company_admin_details`.*, `tbl_company_admin_roles`.`fldRoleName` FROM `tbl_company_admin_details`,`tbl_company_admin_roles` WHERE `tbl_company_admin_details`.`fldAdminRole`=`tbl_company_admin_roles`.`fldRoleKey` AND `tbl_company_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_company_admin_roles`.`fldRoleStatus`!='deleted' AND `fldAdminKey`=" . $key;
