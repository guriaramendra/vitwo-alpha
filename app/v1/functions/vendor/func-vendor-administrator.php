...
function loginAdministratorUser($POST){
    global $dbCon;
    $returnData=[];
    $sql="SELECT * FROM `tbl_vendor_admin_details` WHERE `fldAdminEmail`='".$POST["email"]."' AND `fldAdminStatus`='active'";
    if($result=mysqli_query($dbCon,$sql)){
        if(mysqli_num_rows($result)>0){
            ...
            if($POST["pass"]==$row["fldAdminPassword"]){
                ...
                $_SESSION["logedVendorAdminInfo"]["fldAdminBranchId"]=$row["fldAdminBranchId"];
