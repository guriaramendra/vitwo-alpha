    ...
    if (!isset($_SESSION["logedBranchAdminInfo"]["adminId"]) || !isset($_SESSION["logedBranchAdminInfo"]["adminRole"])) {
        ...
        if ($adminRole != 1) {
            ...
            if (!in_array($currentPage, $commonAccessPages)) {
                $sqlAccesses = "SELECT `fldRoleAccesses` FROM `tbl_branch_admin_roles` WHERE `fldRoleStatus`='active' AND `fldRoleKey`=" . $adminRole;
                $accesses = "";
                ...
                $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_branch_admin_menu` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldMenuFile`='".$currentPage."'";
...
function getAdministratorMenuSubMenu()
{
    ...
    $adminRole = $_SESSION["logedBranchAdminInfo"]["adminRole"];
    if($adminRole==1){
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_branch_admin_menu` WHERE `fldParentMenuKey`!=0 AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
    ...
    if($qryMenuList=mysqli_query($dbCon,$sqlMenuList)){
        if (mysqli_num_rows($qryMenuList)>0) {
            ...
            while($rowMenuList=mysqli_fetch_assoc($qryMenuList)){
                ...
                $sqlSubMenuList="SELECT * FROM `tbl_branch_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=".$parentMenuKey." ORDER BY `fldMenuOrderBy` ASC";
...
function addAdministratorMenu($POST)
{
    ...
    $ins = "INSERT INTO `tbl_branch_admin_menu`
                SET
...
function updateAdministratorMenu($POST)
{
    ...
    if ($res = mysqli_query($dbCon, "SELECT COUNT(`fldMenuLabel`) AS 'noOfRecords' FROM `tbl_branch_admin_menu` WHERE `fldMenuKey`=" . $POST["menuKey"])) {
        if (mysqli_fetch_assoc($res)["noOfRecords"] == 1) {
            $ins = "UPDATE `tbl_branch_admin_menu`
                SET
                    `fldMenuLabel` = '" . $menuLabel . "',
                    `fldMenuIcon` = '" . $menuIcon . "',
                    `fldMenuFile` = '" . $menuFile . "',
...
function getAdministratorMenuList($parentMenuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu` WHERE `fldParentMenuKey`=" . $parentMenuKey . " AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
...
function getAdministratorMenuDetails($menuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu` WHERE `fldMenuKey`=" . $menuKey;
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function addNewAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_branch_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleCompanyId`='" . $fldRoleCompanyId . "' AND `fldAdminBranchId`='" . $fldAdminBranchId . "' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_branch_admin_roles` SET
...
function updateAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_branch_admin_roles` WHERE `fldRoleKey`=" . $roleKey . " AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 1) {

                $checkName = "SELECT * FROM `tbl_branch_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleKey`!=" . $roleKey . " AND  `fldRoleStatus`!='deleted'";
                ...
                $ins = "UPDATE `tbl_branch_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
...
function getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleKey`=" . $key . "";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorRoles()
{
    ...
    $fldRoleCompanyId = $_SESSION["logedBranchAdminInfo"]["fldAdminCompanyId"];
    $fldAdminBranchId = $_SESSION["logedBranchAdminInfo"]["fldAdminBranchId"];
    $sql = "SELECT * FROM `tbl_branch_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleCompanyId`=".$fldRoleCompanyId." AND `fldAdminBranchId`=".$fldAdminBranchId."";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorUsers()
{
    ...
    $fldAdminBranchId = $_SESSION["logedBranchAdminInfo"]["fldAdminBranchId"];
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_details`,`tbl_branch_admin_roles` WHERE `tbl_branch_admin_details`.`fldAdminRole`=`tbl_branch_admin_roles`.`fldRoleKey` AND `tbl_branch_admin_details`.`fldAdminCompanyId`='".$fldAdminCompanyId."' AND `tbl_branch_admin_details`.`fldAdminBranchId`='".$fldAdminBranchId."' AND `tbl_branch_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_branch_admin_roles`.`fldRoleStatus`!='deleted'";
...
function getAdministratorUserDetails($key = null)
{
    ...
    $sql = "SELECT `tbl_branch_admin_details`.*, `tbl_branch_admin_roles`.`fldRoleName` FROM `tbl_branch_admin_details`,`tbl_branch_admin_roles` WHERE `tbl_branch_admin_details`.`fldAdminRole`=`tbl_branch_admin_roles`.`fldRoleKey` AND `tbl_branch_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_branch_admin_roles`.`fldRoleStatus`!='deleted' AND `fldAdminKey`=" . $key;
...
function getAdministratorAccessesNames($roleAccesses = "0")
{
    ...
    if ($roleAccesses == "0") {
        ...
        $sql = "SELECT `fldMenuLabel` FROM `tbl_branch_admin_menu` WHERE `fldMenuKey` IN (" . $roleAccesses . ") AND `fldMenuStatus`='active'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $returnData["status"] = "success";
                $returnData["message"] = "success";
